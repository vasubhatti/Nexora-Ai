...
const UnitTestGenerator = () => {
  const [form, setForm] = useState({ code: "", language: "", framework: "" });
  ...
  const handleSubmit = async () => {
    ...
    setResult(null);
    try {
      const { data } = await api.post("/code/test", form);
  ...
  return (
    <Layout>
      <AIToolLayout title="Unit Test Generator" description="Generate unit tests for your code." cost="4 credits">
        <div className="space-y-4">
          <div>
            ...
            <textarea value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })}
              placeholder="Paste code to generate tests for..." rows={6}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" value={form.framework} onChange={(e) => setForm({ ...form, framework: e.target.value })}
                placeholder="e.g. Jest, Mocha, PyTest"
                className="w-full px-3 py-2.5 bg-zinc-900 border border-zinc-800 rounded-lg text-white placeholder:text-zinc-600 focus:outline-none focus:ring-1 focus:ring-zinc-600 text-sm transition-colors" />
            </div>
          </div>
          <button
